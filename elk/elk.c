...
enum {
    TOK_ERR,
    TOK_EOF, // 表示已经解析到code buf的末尾了。不是表示未见文件尾部。
    TOK_IDENTIFIER,
    TOK_NUMBER,
    TOK_STRING,
    TOK_SEMICOLON,
...
{
    return (c >= '0' && c <= '9') ? (uint8_t)(c - '0') : (c >= 'a' && c <= 'f') ? (uint8_t)(c - 'W')
        : (c >= 'A' && c <= 'F')                                                ? (uint8_t)(c - '7')
                                                                                : 0;
}
...
static size_t tostr(struct js* js, jsval_t value, char* buf, size_t len)
{
    switch (vtype(value)) {
    case T_UNDEF:
        return cpy(buf, len, "undefined", 9);
    case T_NULL:
        return cpy(buf, len, "null", 4);
    case T_BOOL:
        return cpy(buf, len, vdata(value) & 1 ? "true" : "false", vdata(value) & 1 ? 4 : 5);
    case T_OBJ:
    ...
    case T_CFUNC:
        return (size_t)snprintf(buf, len, "\"c_func_0x%lx\"", (unsigned long)vdata(value));
    case T_PROP:
        return (size_t)snprintf(buf, len, "PROP@%lu", (unsigned long)vdata(value));
    default:
...
{
    jsoff_t ofs = js->brk;
    size = align32((jsoff_t)size); // 分配是4字节对齐的。
...
static jsval_t mkobj(struct js* js, jsoff_t parent) // parent为0(NULL)则表示是global的空间。
{
    // mylogd("parent addr:%p", &parent);
...
static inline jsoff_t esize(jsoff_t w)
{
    switch (w & 3U) {
    case T_OBJ:
        return (jsoff_t)(sizeof(jsoff_t) + sizeof(jsoff_t)); // 8个字节
    case T_PROP:
        return (jsoff_t)(sizeof(jsoff_t) + sizeof(jsoff_t) + sizeof(jsval_t)); // 16个字节
    case T_STR:
        return (jsoff_t)(sizeof(jsoff_t) + align32(w >> 2U)); // 4+x字节
    default:
        return (jsoff_t)~0U; // 返回-1
    }
}
...
static uint8_t next(struct js* js)
{
    ...
    js->tlen = 0;
    const char* buf = js->code + js->toff; // buf指向的是有效字符的位置上。

    if (js->toff >= js->clen) { // 已经解析到末尾了。
        js->tok = TOK_EOF;
        return js->tok;
    }
#define TOK(T, LEN)       \
...
static jsval_t do_op(struct js* js, uint8_t op, jsval_t lhs, jsval_t rhs)
{
    ...
    if (is_unary(op) && vtype(r) != T_NUM)
        return js_mkerr(js, "type mismatch");
    if (!is_unary(op) && op != TOK_DOT && (vtype(l) != T_NUM || vtype(r) != T_NUM))
        return js_mkerr(js, "type mismatch");
    double a = tod(l), b = tod(r);
    switch (op) {

    case TOK_DIV:
        return tod(r) == 0 ? js_mkerr(js, "div by zero") : tov(a / b);
    case TOK_REM:
        return tov(a - b * ((double)(long)(a / b)));
    case TOK_MUL:
        return tov(a * b);
    case TOK_PLUS:
        return tov(a + b);
    case TOK_MINUS:
        return tov(a - b);
    case TOK_XOR:
        return tov((double)((long)a ^ (long)b));
    case TOK_AND:
        return tov((double)((long)a & (long)b));
    case TOK_OR:
        return tov((double)((long)a | (long)b));
...
static jsval_t js_literal(struct js* js)
{
    ...
    case TOK_NUMBER:
        return js->tval;
    case TOK_STRING:
        return js_str_literal(js);
    case TOK_LBRACE:
        return js_obj_literal(js);
    case TOK_FUNC:
        return js_func_literal(js);
    case TOK_NULL:
        return js_mknull();
    case TOK_UNDEF:
        return js_mkundef();
...
struct js* js_create(void* buf, size_t len)
{
    ...
    memset(buf, 0, len);
    js = (struct js*)buf;
    js->mem = (uint8_t*)(js + 1); //+1表示把最前面的struct js的内存空间留出来。
    js->size = (jsoff_t)(len - sizeof(*js));
    js->scope = mkobj(js, 0); // 命名空间。默认是全局的，global。
...
void js_setgct(struct js* js, size_t gct)
{
    js->gct = (jsoff_t)gct;
}
void js_setmaxcss(struct js* js, size_t max)
{
...
jsval_t js_eval(struct js* js, const char* buf, size_t len)
{
    ...
    jsval_t res = js_mkundef(); // 先构造一个非法的返回值，就相当于平时我们写的int ret = -1.
    if (len == (size_t)~0U)
        len = strlen(buf);
    js->consumed = 1; // 默认给1
    js->tok = TOK_ERR; // 给一个非法值。
    js->code = buf;
    js->clen = (jsoff_t)len;
    js->pos = 0; // 当前解析的位置就是0
    js->cstk = &res; // 当前cstk的位置。
    while (next(js) != TOK_EOF && !is_err(res)) { // 如果没有解析完，且没有执行错误。
        res = js_stmt(js); // 那么继续执行语句。
    }
    return res;
}
