...
我喜欢C语言，喜欢Python和lua，所以会集成micropython和lua。不喜欢C++，所以不会有一行C++代码出现在本项目里。喜欢一切小巧的常用库实现。
...
| 脚本语言库      |          |                                                              |
| micropython     | √        | 描述：就是官方的micropython仓库，相比于python，这个的大小更加可控。我会只保留代码里的Linux需要的部分，其余代码一律删除。必须要说，当前加进来的只是我进行研读阅读用的。编译出来的可执行文件，只包含了sys和uos这2个模块。不具备实用性。而把完整的micropython编译进来，需要对编译做更多的调整。 |
