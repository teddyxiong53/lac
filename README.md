...
我喜欢C语言，喜欢Python和lua，所以会集成~~micropython~~（改成pikapython了，这个更加简单易懂，对我够用了）和lua。喜欢一切小巧的常用库实现。
...
为什么是TCP？这个选择受陈硕的影响。TCP稳定可靠，可以通过抓包工具来抓包调试问题，如果有需要，可以很容易从单机扩展到多机。
...
| 脚本语言库      |          |                                                              |
| micropython     | x        | 描述：就是官方的micropython仓库，相比于python，这个的大小更加可控。我会只保留代码里的Linux需要的部分，其余代码一律删除。必须要说，当前加进来的只是我进行研读阅读用的。编译出来的可执行文件，只包含了sys和uos这2个模块。不具备实用性。而把完整的micropython编译进来，需要对编译做更多的调整。<br/>删掉了。不用这个了。 |
| pikapython      | √        | 一个轻量级的python实现，主要用在mcu上。适合用来研究python解释器的简单实现。 |
| lua             | √        | 描述：就是lua官方仓库，因为这个官方版本就足够精简了。这个全部集成进来，自己写编译代码。 |
